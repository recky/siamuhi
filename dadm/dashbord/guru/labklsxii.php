...
<script type="text/javascript">
    ...
    function cari(){
        $('#key').change(function(){
            ...
            var key =$("#key").val();
            //alert($("#ThnAjar").val());
            window.location="index.php?content=abklsxii&key="+key;
        });
    }
</script>
    <?php
...
<form method="post" action="../dashbord/guru/inproab.php" enctype="multipart/form-data">
    ...
    <table width="100%" class="table table-bordered table-striped">
        <tr><td colspan="2" align="right">Kelas : <?php echo"$_GET[key]"; ?> </td>
            ...
            <?php
            $qkls=mysql_query("SELECT *FROM dafkelas WHERE substr( IdKelas, 1, 2 ) = '12'");
            while($rows=mysql_fetch_array($qkls)){
                echo"<option value=$rows[IdKelas]>$rows[IdKelas]</option>";
            }
            ?>
        </select>
        </td><td><a href="?content=abklsxii&mod=l" class="btn btn-success"><i class="icon-zoom-in"></i>Lihat</a></td></tr>
