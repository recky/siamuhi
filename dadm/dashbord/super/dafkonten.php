...
<script type="text/javascript">
    function hapus(id){
        ...
        if(cek==true){
            window.location='../dashbord/super/aksiktn.php?id='+id+'&aksi=hapus';
        }else{
...
<table width="100%" class="table table-bordered table-striped">
    ...
    if($cr!=''){
        switch($key){
            case 'judul':$query="Select * from info_konten where judul like '%$cr%' order by tgl desc";
                break;
            case 'penulis':$query="Select * from info_koten where nama like '%$cr%' order by tgl desc";
                break;
            case 'hak':$query="Select * from info_konten where hak like '%$cr%' order by tgl desc";
                break;
            case 'publikasi':
                switch($cr){
                    case "ya":$st=1;
                ...
                $query="Select * from info_konten where status=$st order by tgl desc";
                break;
        }

    }else{$query="Select * from info_konten order by tgl desc";}
    ...
    while($row=mysql_fetch_array($hasil))
    {
        ...
        echo"<tr><td>$no</font></td>".
            ...
            "<td width='10%'> <a href='#' onclick='hapus($row[id])' class='btn btn-xs btn-danger' title='Hapus'><i class='icon-trash'></i></a> <a href='?content=editor&id=$row[id]&edit=1' class='btn btn-xs btn-primary' title='Edit'><i class='icon-pencil'></i></a> <a href='../dashbord/super/aksiktn.php?id=$row[id]&aksi=unpub' class='btn btn-xs btn-warning' title='Publish/Unpublish'><i class='icon-ban-circle'></i></a> </td></tr>";
        $no=$no+1;
    }
...
<?php
$jmldata = mysql_num_rows(mysql_query("Select * from info_konten order by tgl desc"));
