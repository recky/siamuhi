...
<div class="main-content">
    ...
    <div class="page-content">
        <div class="container-fluid">
            <div class="row-fluid">
                    <?php if($_GET['edit']==1){?>
                <form class="form-horizontal  well" method='post' action='../dashbord/staff/ejadwal.php' enctype="multipart/form-data">
                    <center><h3>Edit Daftar Pelajaran Pada  Kelas</h3>
                        <table>
                            <tr><td></td><td>
                                <div class="control-group">
                                    <div class="controls">
                                        <input type="text" value="<?php echo $_GET['id'];?>" hidden="hidden" name="id"/>
                                        <select tabindex="5" class="chzn-select input-xlarge" name="kelas" data-placeholder="Pilih Kelas">
                                            <option value=""></option>
                                            <?php
                                            include ("konek.php");
                                            $qs=mysql_query("select * from jadwal where id='$_GET[id]'");
                                            $hcr=mysql_fetch_array($qs);
                                            $qnik="select * from dafkelas";
                                            $hslnik=mysql_query($qnik);
                                            while($rownik=mysql_fetch_array($hslnik)){
                                                if($hcr['IdKelas']==$rownik['IdKelas']){$tnd="selected=selected";}else{$tnd="";}
                                                echo "<option name='kelas' value='$rownik[IdKelas]'".$tnd.">".$rownik[IdKelas]."</option>";
                                            };
            ...
            <?}else{?>
                    <form class="form-horizontal  well" method='post' action='../dashbord/staff/injadwal.php' enctype="multipart/form-data">
                        <center><h3>Input Pelajaran pada Kelas</h3>
                    <table>
                        <tr><td></td><td>
                            <div class="control-group">
                            <div class="controls">
                                ...
                                <select tabindex="5" class="chzn-select input-xlarge" name="kelas"data-placeholder="Pilih Kelas">
                                    ...
                                    $hslnik=mysql_query($qnik);
                                    while($rownik=mysql_fetch_array($hslnik)){
                                        echo "<option name='kelas' value='$rownik[IdKelas]'>".$rownik[IdKelas]."</option>";
                                    };
