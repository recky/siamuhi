...
<script type="text/javascript">
    ...
    function edit(id){
     window.location='../dashbord/staff/acc.php?id='+id;
    }
</script>
...
<table width="100%" class="table table-bordered table-striped">
    ...
    while($row=mysql_fetch_array($hasil))
    {
        ...
        echo"<tr><td>$row[jenis]$row[kelas]$row[notrans]</font></td>".
            ...
            "<td>$row[teller]</td>".
            "<td>$jk</td>";?>
            <td width="10%"><a href="#" onclick="edit('<?php echo $t?>')" class="btn btn-xs btn-danger" title="Setuju"><i class='icon-check'></i> Setuju</a></td></tr>
                <?php
        $no=$no+1;
    }
...
<?php
$jmldata = mysql_num_rows(mysql_query("Select *,info_bayar.status as stus from info_bayar,info_form,ta t where info_bayar.nis=info_form.nis and info_bayar.teller!='' and info_bayar.ta=t.ta and t.status=1 order by tanggal desc"));
